...
namespace Ferry_Ticketing_App.Pages
{
    public partial class ucOneWTripPayment : UserControl
    {
        ...
        private void UpdateTotalPrice(decimal charge)
        {
            try
            {
                ...
                {
                    decimal basePrice = decimal.Parse(searchOneWayTrip.ucDepartureSummary1.lblDPrice.Text.Replace("₱", ""));
                    ...
                    if (passengerContactInfo != null)
                    {
                        ...
                        foreach (var passenger in passengerDetails)
                        {
                            ...
                            decimal individualPrice = price.ApplyDiscount(passengerType, age);
                            totalPrice += individualPrice;
                        }
                    }

                    CalculateIndividualPrices(basePrice);
                }

                totalPrice += (terminalFee + charge);
        ...
        public void SetupPassengerInfo(int numberOfPassengers, List<Passenger> passengers)
        {
            ...
            {
                SetPassengerInfo(firstPassengerInfo, passengers[0], 1);
            }
            ...
            for (int i = 2; i <= numberOfPassengers; i++)
            {
                ...
                {
                    SetPassengerInfo(newPassengerInfo, passengers[i - 1], i);
                }
        ...
        private void btnPaymentContinue_Click(object sender, EventArgs e)
        {
            ...
            if (checkOut == null)
            {
                MessageBox.Show("Unable to navigate to the checkout page.", 
            ...
            if (pnlCheckout == null)
            {
                MessageBox.Show("Unable to find the checkout panel.", 
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        public void CalculateIndividualPrices(decimal basePrice)
        {
            try
            {
                var passengerContactInfo = this.Parent.Controls.OfType<ucPassengerContactInfo>().FirstOrDefault();
                ...
                foreach (var passenger in passengerDetails)
                {
                    ...
                    // Calculate individual price with discount
                    Price price = new Price(basePrice, 0);
                    decimal individualPrice = price.ApplyDiscount(passengerType, age);
                    totalDiscount += (basePrice - individualPrice);
                }
                ...
                decimal discountPercentage = (totalDiscount / (basePrice * passengerDetails.Count)) * 100;
